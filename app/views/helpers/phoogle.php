...
/**
* showPanoramio
* True/False whether to show Panoramio imagen on map or not
*/    
    var $showPanoramio = false;
...
*/
	function showMap(){
		...
		if ($this->showControl){
          ...
          if ($this->controlType == 'large'){echo "map.addControl(new GLargeMapControl());\n";}
                ...
                if( $this->showPanoramio){
                    echo 'map.addOverlay(new GLayer("com.panoramio.all"));';
                }
 ...
 ///////////THIS BLOCK OF CODE IS FROM Roger Veciana's CLASS (assoc_array2xml) OBTAINED FROM PHPCLASSES.ORG//////////////
   	function xml2array($xml){
		...
		//xml_parser_set_option ($this->xml_parser,XML_OPTION_SKIP_WHITE,1);//Sigh
    ...
    function startElement($parser, $name, $attrs){
		   ...
		   $this->node_flag=1;
		   $this->depth++;
		   //$this->log("startElement(\$parser, $name, $attrs): " . var_export($this->arrays, true), LOG_DEBUG);
     }
    function characterData($parser,$data){
       ...
       $this->node_flag=0;
       //$this->log("characterData(\$parser, $data): " . var_export($this->arrays, true), LOG_DEBUG);
     }
    function endElement($parser, $name)
     {
       ...
       $this->node_flag=1;
       $this->depth--;
       //$this->log("endElement(\$parser, $name): " . var_export($this->arrays, true), LOG_DEBUG);
     }
